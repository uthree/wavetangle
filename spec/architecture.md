...
│   ├── mod.rs           # DSPモジュールのルート、共通関数
│   ├── biquad.rs        # Biquadフィルター
│   ├── compressor.rs    # コンプレッサー
│   ├── spectrum.rs      # スペクトラムアナライザー
...
### WORLDボコーダー (world_vocoder.rs)
- `WorldVocoder`: WORLDアルゴリズムによる声質変換（ピッチシフト + フォルマントシフト）
  - world-dspクレートを使用（YIN F0推定、CheapTrickスペクトル包絡、D4C非周期性指標、Synthesizer再合成）
  - 50%オーバーラップ + Hann窓によるoverlap-add方式（ブロック境界での音途切れ防止）
  - ワーカースレッドで非同期処理（エフェクトプロセッサスレッドをブロックしない）
  - 合成結果をリサンプルして入力と同一サンプル数を保証（入出力レート一致）
  - パススルーフォールバック（ワーカーが間に合わない場合は入力信号をそのまま出力）
  - ブロックサイズ可変（40〜200ms、UIから調整可能）

### グラフィックEQ (graphic_eq.rs)
...
### 補間アルゴリズム (interpolation.rs)
- `Interpolator` trait: 補間アルゴリズムの抽象化
  - `interpolate()`: バッファから指定位置のサンプルを補間
  - `required_margin()`: 補間に必要なサンプル数
  - `name()`: 補間器の名前
