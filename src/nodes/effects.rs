...
// ============================================================================
// TdPsolaPitchShift Node - TD-PSOLAピッチシフト（フォルマント独立制御）
// ============================================================================

/// TD-PSOLAピッチシフトノード
/// ピッチとフォルマントを独立に制御可能
pub struct TdPsolaPitchShiftNode {
    /// ピッチシフト量（半音単位、-24〜+24）
    pub pitch_shift: f32,
...
impl Default for TdPsolaPitchShiftNode {
    fn default() -> Self {
        Self::new()
    }
}

// TdPsolaPitchShiftNodeのトレイト実装
impl NodeBase for TdPsolaPitchShiftNode {
    fn node_type(&self) -> NodeType {
...
impl_input_port_nb!(TdPsolaPitchShiftNode, ["In"]);
impl_single_output_port_nb!(TdPsolaPitchShiftNode);

impl NodeUI for TdPsolaPitchShiftNode {
    fn is_active(&self) -> bool {
        self.is_active
    }
    ...
    fn show_body(&mut self, ui: &mut Ui, _ctx: &NodeUIContext) {
        ui.vertical(|ui| {
            ...
            ui.add(egui::Slider::new(&mut self.pitch_shift, -24.0..=24.0).suffix(" st"));

            // ピッチ表示（オクターブ + 半音 + セント）
            let octaves = (self.pitch_shift / 12.0).trunc() as i32;
            let semitones = (self.pitch_shift % 12.0).trunc() as i32;
            let cents = ((self.pitch_shift.fract()) * 100.0).round() as i32;
            if octaves != 0 {
                ui.label(format!(
                    "{:+} oct, {:+} st, {:+} cents",
                ...
                ui.label(format!("{:+} st, {:+} cents", semitones, cents));
            }
            ...
            ui.add(egui::Slider::new(&mut self.formant_shift, -24.0..=24.0).suffix(" st"));
            ...
            ui.collapsing("Advanced", |ui| {
                ...
                if ui
                    .add(egui::Slider::new(&mut self.yin_threshold, 0.05..=0.5).fixed_decimals(2))
                    .changed()
                {
                    if let Some(mut td_psola) = self.td_psola.try_lock() {
                        td_psola.set_yin_threshold(self.yin_threshold);
                    }
                }
